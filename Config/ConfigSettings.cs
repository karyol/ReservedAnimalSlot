...
namespace ReservedAnimalSlot.Config
{
    public static class ConfigSettings
    {
        ...
        {
            ReservedAnimalSlot.Log("BindingConfigs");
            
            overrideItemSlotPriority = AddConfigEntry(ReservedAnimalSlot.Instance.Config.Bind("Server-side", "CatSlotPriorityOverride", 100, "[Host only] Manually set the priority for this item slot. Higher priority slots will come first in the reserved item slots, which will appear below the other slots. Negative priority items will appear on the left side of the screen, this is disabled in the core mod's config."));
            overridePurchasePrice = AddConfigEntry(ReservedAnimalSlot.Instance.Config.Bind("Server-side", "CatSlotPriceOverride", 200, "[Host only] Manually set the price for this item in the store. Setting 0 will force this item to be unlocked immediately after the game starts."));
            additionalItemsInSlot = AddConfigEntry(ReservedAnimalSlot.Instance.Config.Bind("Server-side", "AdditionalItemsInSlot", "", "[Host only] Syntax: \"Item1,Item name2\" (without quotes). When adding items, use the item's name as it appears in game. Include spaces if there are spaces in the item name. Adding items that do not exist, or that are from a mod which is not enabled will not cause any problems.\nNOTE: IF YOU ARE USING A TRANSLATION MOD, YOU MAY NEED TO ADD THE TRANSLATED NAME OF ANY ITEM YOU WANT IN THIS SLOT."));
        ...
        public static void TryRemoveOldConfigSettings()
        {
            ...
            try
            {
                ConfigFile config = ReservedAnimalSlot.Instance.Config;
